...
import cc.wutao.ai.properties.AiProperties;
import cc.wutao.ai.service.AiSummaryGeneratorImpl;
import cc.wutao.ai.service.AiTypoCorrectorImpl;
 ...
 * AI 模块自动装配
 *
 * 仅当 wutao.ai.enabled=true 时注册各 AI 功能实现 Bean；
 * 未启用或配置缺失时本配置类不生效，应用照常启动（fail-safe）。
 */
@Configuration
@ConditionalOnProperty(prefix = "wutao.ai", name = "enabled", havingValue = "true")
